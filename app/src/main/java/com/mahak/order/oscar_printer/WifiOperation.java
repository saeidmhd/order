...
public class WifiOperation implements IPrinterOpertion {
    ...
    //Check the status every 10s. Can check to prove that the connection is normal.
    // Otherwise check again. Not yet disconnected
    ...
    public PrinterInstance getPrinter() {
        if (mPrinter != null && mPrinter.isConnected()) {
            timer.schedule(myTask, 5000, 100000);
        }
        return mPrinter;
    }
